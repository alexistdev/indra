    ...
    <div class="row justify-content-center">
        <div class="col-lg-5">
            <div class="card shadow-lg border-0 rounded-lg mt-3">
                ...
                <div class="card-body">
                    <form action="<?= base_url('produk/editProduk/'.$produk['id']); ?>" method="POST">
                        <div class="form-group">
                            <label for="kategoriId">Kategori</label>
                            <select name="kategoriId" class="form-control" id="kategoriId" required>
                                <option value="">--pilih kategori--</option>
                                <?php foreach($kategori as $data) : ?>
                                <option value="<?= $data['id']; ?>" <?= ($idKategori == $data['id'])?"selected":""; ?>><?= $data['kategori']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="merkId" class="form-control" id="merkId" required>
                                ...
                                <option value="<?= $data['id']; ?>" <?= ($idMerk == $data['id'])?"selected":"" ?>><?= $data['merk']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="sizeId" class="form-control" id="sizeId" required>
                                ...
                                <option value="<?= $data['id']; ?>" <?= ($idSize == $data['id']) ? ' selected="selected"' : ''?>><?= $data['size']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="kode_produk">Kode Produk</label>
                            <input name="kode_produk" type="text" class="form-control" id="kode_produk" placeholder="input nama kode produk..." value="<?= $produk['kode_produk']; ?>" required>
                            <?php echo form_error('kode_produk', '<div class="error alert-danger">', '</div>'); ?>
                        </div>
                        <div class="form-group">
                            <label for="type">Type</label>
                            <input name="type" type="text" class="form-control" id="type" placeholder="input nama type..." value="<?= $produk['type']; ?>" required>
                            <?php echo form_error('type', '<div class="error alert-danger">', '</div>'); ?>
                        </div>
                        <div class="form-group">
                            <label for="harga">Harga</label>
                            <input name="harga" type="number" class="form-control" id="harga" placeholder="input harga..." value="<?= $produk['harga']; ?>" required>
                            <?php echo form_error('harga', '<div class="error alert-danger">', '</div>'); ?>
                        </div>
                        <div class="form-group">
                            <label for="stok">Jumlah Stok</label>
                            <input name="stok" type="number" class="form-control" id="stok" placeholder="input stok..." value="<?= $produk['stok']; ?>" required>
